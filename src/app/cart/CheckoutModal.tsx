...
const CheckoutModal: React.FC<CheckoutModalProps> = ({
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			if (!response.ok) {
				let errorMsg = 'Произошла ошибка при отправке заказа'
				try {
			...
			// setCartItems([]); — это надо делать в родительском компоненте
		} catch (error) {
			const errorMessage =
				error instanceof Error ? error.message : 'Неизвестная ошибка.'
			console.error('Ошибка при отправке заказа:', errorMessage)
			setSubmitError(errorMessage)
		} finally {
			setIsSubmitting(false)
		}
					...
					<>
						...
						<div className={styles.modalBody}>
							...
							<form onSubmit={handleSubmit}>
								...
								<div className={styles.formGroup}>
									<label htmlFor='notes'>
										Примечания к вашему заказу (необязательно)
									</label>
									<textarea
										id='notes'
										name='notes'
